...
function showMoveSubmenu(anchorItem, sessionId, projects) {
  // Toggle: remove existing submenu if already showing
  var existing = anchorItem.parentElement.querySelector(".session-ctx-submenu");
  if (existing) { existing.remove(); return; }

  // Insert project buttons inline below the "Move to..." item
  var sub = document.createElement("div");
  sub.className = "session-ctx-submenu";

  for (var i = 0; i < projects.length; i++) {
    (function (p) {
      var btn = document.createElement("button");
      btn.className = "session-ctx-item session-ctx-move-target";
      btn.textContent = p.title || p.project || p.slug;
  ...
  // Insert submenu right after the move button in the menu
  anchorItem.insertAdjacentElement("afterend", sub);
}
...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid) {
  ...
  // "Move to..." — only show when multiple projects exist
  var otherProjects = (ctx.cachedProjects || []).filter(function (p) {
    return p.slug !== ctx.currentSlug;
  });
  if (otherProjects.length > 0) {
    var moveItem = document.createElement("button");
    moveItem.className = "session-ctx-item session-ctx-move";
    var moveIcon = document.createElement("span");
    moveIcon.insertAdjacentHTML("afterbegin", iconHtml("arrow-right"));
    moveItem.appendChild(moveIcon.firstChild);
    var moveLabel = document.createElement("span");
    moveLabel.textContent = "Move to\u2026";
    moveItem.appendChild(moveLabel);
    moveItem.addEventListener("click", function (e) {
      e.stopPropagation();
      showMoveSubmenu(moveItem, sessionId, otherProjects);
    });
...
export function updatePageTitle() {
  ...
  if (ctx.projectName && sessionTitle) {
    ...
    document.title = ctx.projectName + " - Braigi";
  } else {
    document.title = "Braigi";
  }
}
...
export function initSidebar(_ctx) {
  ...
  // --- CLI sessions browser ---
  var cliModal = ctx.$("cli-sessions-modal");
  var cliListEl = ctx.$("cli-sessions-list");
  var cliEmptyEl = ctx.$("cli-sessions-empty");
  var cliLoadingEl = ctx.$("cli-sessions-loading");
  var cliSearchInput = ctx.$("cli-sessions-search-input");
  var cliCloseBtn = ctx.$("cli-sessions-close");
  var browseCliBtn = ctx.$("browse-cli-btn");
  var cliSessionsData = [];
  ...
  function renderCliSessions(filter) {
    // Clear list using safe DOM removal
    while (cliListEl.firstChild) cliListEl.removeChild(cliListEl.firstChild);

    var q = (filter || "").toLowerCase();
    var filtered = cliSessionsData;
    if (q) {
      filtered = cliSessionsData.filter(function (s) {
        return s.title.toLowerCase().indexOf(q) !== -1;
      });
    }

    if (filtered.length === 0) {
      cliEmptyEl.textContent = q ? "No sessions match your filter." : "No CLI sessions found for this project.";
      cliEmptyEl.classList.remove("hidden");
      return;
    }
    cliEmptyEl.classList.add("hidden");
    ...
    for (var i = 0; i < filtered.length; i++) {
      ...
      if (group !== currentGroup) {
        currentGroup = group;
        var header = document.createElement("div");
        header.className = "cli-sessions-group-header";
        header.textContent = group;
        cliListEl.appendChild(header);
      }

      var row = document.createElement("div");
      row.className = "cli-session-row";

      var info = document.createElement("div");
      info.className = "cli-session-info";

      var titleEl = document.createElement("div");
      titleEl.className = "cli-session-title";
      titleEl.textContent = s.title;
      info.appendChild(titleEl);

      var meta = document.createElement("div");
      meta.className = "cli-session-meta";
      meta.textContent = formatRelativeDate(s.lastActivity) + " \u00B7 " + formatSize(s.size);
      ...
      if (s.active) {
        ...
        badge.className = "cli-session-badge";
      ...
      cliListEl.appendChild(row);
    }
  }

  function openCliModal() {
    if (!cliModal || !cliListEl) return;
    cliModal.classList.remove("hidden");
    if (cliSearchInput) cliSearchInput.value = "";
    while (cliListEl.firstChild) cliListEl.removeChild(cliListEl.firstChild);
    if (cliEmptyEl) cliEmptyEl.classList.add("hidden");
    if (cliLoadingEl) cliLoadingEl.classList.remove("hidden");
    if (cliSearchInput) setTimeout(function () { cliSearchInput.focus(); }, 50);

    var url = ctx.basePath + "api/cli-sessions";
    fetch(url)
      .then(function (r) {
      ...
      .then(function (data) {
        if (cliLoadingEl) cliLoadingEl.classList.add("hidden");
        cliSessionsData = data.sessions || [];
        renderCliSessions("");
        refreshIcons();
      })
      .catch(function (err) {
        console.error("CLI sessions fetch failed:", err);
        if (cliLoadingEl) cliLoadingEl.classList.add("hidden");
        if (cliEmptyEl) {
          cliEmptyEl.textContent = "Failed to load CLI sessions.";
          cliEmptyEl.classList.remove("hidden");
        }
      });
  }
  ...
  if (browseCliBtn) {
    browseCliBtn.addEventListener("click", openCliModal);
  }
  if (cliCloseBtn) {
    cliCloseBtn.addEventListener("click", closeCliModal);
  }
  if (cliModal) {
    cliModal.querySelector(".confirm-backdrop").addEventListener("click", closeCliModal);
  }
  if (cliSearchInput) {
    cliSearchInput.addEventListener("input", function () {
      renderCliSessions(cliSearchInput.value.trim());
    });
    cliSearchInput.addEventListener("keydown", function (e) {
      if (e.key === "Escape") {
        e.preventDefault();
        closeCliModal();
      }
