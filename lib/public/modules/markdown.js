  ...
  blocks.forEach(function (codeEl) {
    ...
    try {
      mermaid.render(id, source.trim()).then(function (result) {
        var sanitizedSvg = DOMPurify.sanitize(result.svg, { USE_PROFILES: { svg: true, svgFilters: true } });
        container.textContent = "";
        container.insertAdjacentHTML("afterbegin", sanitizedSvg);
        container.addEventListener("click", function () {
          showMermaidModal(container.firstElementChild ? container.firstElementChild.outerHTML : "");
        });
...
export function showMermaidModal(svgHtml) {
  ...
  var sanitizedSvg = DOMPurify.sanitize(svgHtml, { USE_PROFILES: { svg: true, svgFilters: true } });
  body.textContent = "";
  body.insertAdjacentHTML("afterbegin", sanitizedSvg);
