...
import { cancelRecording } from './voice.js';
...
var messageQueue = []; // [{text, images?, pastes?}]
...
function safeSend(payload) {
  try {
    if (ctx.ws && ctx.ws.readyState === WebSocket.OPEN) {
      ctx.ws.send(typeof payload === "string" ? payload : JSON.stringify(payload));
      return true;
    }
  } catch (e) {}
  return false;
...
export function sendMessage() {
  ...
  if (!safeSend(payload)) {
    ctx.addSystemMessage("Failed to send — connection lost.", true);
    return;
  }
...
function queueMessage() {
  ...
  if (messageQueue.length >= MAX_QUEUE_SIZE) return;
  hideSlashMenu();

  var entry = { text: text || "" };
  if (images.length > 0) entry.images = images;
  if (pastes.length > 0) entry.pastes = pastes;
...
export function drainQueue() {
  ...
  ctx.addUserMessage(entry.text, entry.images || null, entry.pastes || null);
  var payload = { type: "message", text: entry.text };
  if (entry.images) payload.images = entry.images;
  if (entry.pastes) payload.pastes = entry.pastes;
  safeSend(payload);
  return true;
}
...
export function syncQueueBtn() {
  updateQueueBtn();
}

function renderQueueChips() {
  var container = document.getElementById("queue-chips");
  if (!container) return;
  ...
  for (var i = 0; i < messageQueue.length; i++) {
    (function (idx) {
      ...
      num.className = "queue-chip-num";
      num.textContent = "#" + (idx + 1);
      var preview = document.createElement("span");
      preview.className = "queue-chip-preview";
      var text = messageQueue[idx].text || "";
...
function showSlashMenu(filter) {
  ...
  ctx.slashMenu.innerHTML = slashFiltered.map(function (c, i) {
    return '<div class="slash-item' + (i === 0 ? ' active' : '') + '" data-idx="' + i + '">' +
      '<span class="slash-cmd">/' + escapeHtml(c.name) + '</span>' +
...
function sendInputSync() {
  ...
  inputSyncTimer = setTimeout(function () {
    inputSyncTimer = null;
    safeSend({ type: "input_sync", text: ctx.inputEl.value });
  }, 100);
}
