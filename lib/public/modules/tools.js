...
export function addTurnMeta(cost, duration, timing) {
  ...
  if (parts.length === 0 && !timing) return;

  div.textContent = parts.join(" \u00b7 ");

  if (timing) {
    var timingBtn = document.createElement("span");
    timingBtn.className = "timing-toggle";
    timingBtn.textContent = " \u00b7 " + (timing.total / 1000).toFixed(1) + "s e2e";
    timingBtn.title = "Click to expand timing breakdown";

    var timingDetail = document.createElement("div");
    timingDetail.className = "timing-detail";

    var lines = [];
    lines.push("Overhead: " + timing.overhead + "ms");
    if (timing.ttft != null) lines.push("TTFT: " + timing.ttft + "ms");
    if (timing.toolCount > 0) {
      lines.push("Tools: " + timing.toolCount + " calls, " + (timing.toolTime / 1000).toFixed(1) + "s");
      var showCount = Math.min(timing.tools.length, 20);
      for (var i = 0; i < showCount; i++) {
        lines.push("  \u251c " + timing.tools[i].name + ": " + timing.tools[i].duration + "ms");
      }
      if (timing.tools.length > 20) lines.push("  \u2514 +" + (timing.tools.length - 20) + " more");
    }
    var apiTime = (timing.total || 0) - (timing.toolTime || 0) - (timing.overhead || 0);
    if (apiTime > 0) lines.push("API/thinking: " + (apiTime / 1000).toFixed(1) + "s");

    timingDetail.textContent = lines.join("\n");

    timingBtn.addEventListener("click", function() {
      timingDetail.classList.toggle("visible");
    });

    div.appendChild(timingBtn);
    div.appendChild(timingDetail);
  }
