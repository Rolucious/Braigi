...
var relay = createServer({
  ...
  onAddProject: function (projPath, title) {
    var absPath = path.resolve(projPath);
    if (!fs.existsSync(absPath)) {
      return { ok: false, error: "Directory does not exist: " + absPath, status: 400 };
    }
    try {
      var stat = fs.statSync(absPath);
      if (!stat.isDirectory()) {
        return { ok: false, error: "Path is not a directory: " + absPath, status: 400 };
      }
    } catch (e) {
      return { ok: false, error: "Cannot access path: " + absPath, status: 400 };
    }
