...
var CONFIG_DIR = path.join(os.homedir(), ".braigi");
var RC_PATH = path.join(os.homedir(), ".braigi-rc");
...
// --- ~/.braigi-rc (recent projects persistence) ---
...
function loadRc() {
  try {
    var data = fs.readFileSync(RC_PATH, "utf8");
    return JSON.parse(data);
  } catch (e) {
...
function saveRc(rc) {
  var tmpPath = RC_PATH + ".tmp";
  fs.writeFileSync(tmpPath, JSON.stringify(rc, null, 2) + "\n");
  fs.renameSync(tmpPath, RC_PATH);
}

/**
 * Update ~/.braigi-rc with the current project list from daemon config.
 * Merges with existing entries (preserves addedAt, updates lastUsed).
 */
function syncRc(projects) {
  var rc = loadRc();
