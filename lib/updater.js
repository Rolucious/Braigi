...
function fetchLatestVersion() {
  return new Promise(function (resolve) {
    var req = https.get("https://registry.npmjs.org/braigi/latest", function (res) {
      var data = "";
...
function performUpdate() {
  try {
    execSync("npm install -g braigi@latest", { stdio: "pipe" });
    return true;
  } catch (e) {
    return false;
  }
...
async function checkAndUpdate(currentVersion, skipUpdate) {
  ...
  log(sym.bar + "  " + a.dim + "Run manually: npm install -g braigi@latest" + a.reset);
  log("");
  return false;
}
