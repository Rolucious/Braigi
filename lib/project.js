...
var { createCodexBridge } = require("./codex-bridge");
...
function createProjectContext(opts) {
  ...
  var moveSessionToProject = opts.moveSessionToProject || null;
  ...
  fetchLatestVersion().then(function (v) {
    ...
    log(slug, "version check failed: " + (err && err.message || "unknown"));
  });
  ...
  function handleConnection(ws) {
    ...
    var activeForConn = sm.getActiveSession();
    if (activeForConn && activeForConn.activeBackend === "codex") {
      sendTo(ws, { type: "backend_changed", backend: "codex" });
    }
    ...
    sendTo(ws, {
      ...
      sessions: [].concat(Array.from(sm.sessions.values())).map(function (s) {
        return {
          ...
          activeBackend: s.activeBackend || "claude",
        };
      }),
    });
  ...
  function handleMessage(ws, msg) {
    try {
      handleMessageInner(ws, msg);
    } catch (err) {
      log(slug, "handleMessage error: " + err.message + "\n" + err.stack);
      try { sendTo(ws, { type: "error", text: "Internal error processing message" }); } catch (_) {}
    }
  }

  function handleMessageInner(ws, msg) {
    ...
    if (msg.type === "delete_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        // Clean up takeover terminal before deleting session
        var delSession = sm.sessions.get(msg.id);
        if (delSession.takenOver && delSession.takeoverTermId) {
          tm.close(delSession.takeoverTermId);
        }
    ...
    if (msg.type === "move_session") {
      if (!msg.id || !msg.targetSlug) {
        sendTo(ws, { type: "move_session_result", ok: false, error: "Missing session id or target project" });
        return;
      }
      if (!moveSessionToProject) {
        sendTo(ws, { type: "move_session_result", ok: false, error: "Session moving not available" });
        return;
      }
      var moveResult = moveSessionToProject(slug, msg.id, msg.targetSlug);
      sendTo(ws, {
        type: "move_session_result",
        ok: moveResult.ok,
        error: moveResult.error || null,
        targetSlug: moveResult.targetSlug || null,
      });
      return;
    }
    ...
    if (msg.type === "ask_user_response") {
      ...
      sm.sessions.forEach(function (s) {
        if (!pending && s.pendingAskUser && s.pendingAskUser[toolId]) {
          foundSession = s;
          pending = s.pendingAskUser[toolId];
        }
      });
      if (!foundSession || !pending) return;
      delete foundSession.pendingAskUser[toolId];
      sm.sendAndRecord(foundSession, { type: "ask_user_answered", toolId: toolId });
      pending.resolve({
        behavior: "allow",
    ...
    if (msg.type === "permission_response") {
      ...
      var pending = null;
      sm.sessions.forEach(function (s) {
        if (!pending && s.pendingPermissions && s.pendingPermissions[requestId]) {
          foundSession = s;
          pending = s.pendingPermissions[requestId];
        }
      });
      if (!foundSession || !pending) return;
      delete foundSession.pendingPermissions[requestId];

      if (decision === "allow" || decision === "allow_always") {
        if (decision === "allow_always") {
          if (!foundSession.allowedTools) foundSession.allowedTools = {};
          foundSession.allowedTools[pending.toolName] = true;
          sm.saveSessionFile(foundSession);
        }
    ...
    if (msg.type === "fs_list") {
      ...
      fs.readdir(fsDir, { withFileTypes: true }, function (err, items) {
        if (err) {
          sendTo(ws, { type: "fs_list_result", path: msg.path, entries: [], error: err.message });
          return;
        }
        var entries = [];
    ...
    if (msg.type === "fs_read") {
      ...
      fs.stat(fsFile, function (err, stat) {
        if (err) {
          sendTo(ws, { type: "fs_read_result", path: msg.path, error: err.message });
          return;
        }
        ...
        fs.readFile(fsFile, "utf8", function (readErr, content) {
          if (readErr) {
            sendTo(ws, { type: "fs_read_result", path: msg.path, error: readErr.message });
            return;
          }
    ...
    // --- Terminal takeover ---
    if (msg.type === "takeover_terminal") {
      var session = sm.getActiveSession();
      if (!session || !session.cliSessionId || !UUID_RE.test(session.cliSessionId)) {
        sendTo(ws, { type: "error", text: "No valid CLI session to take over" });
        return;
      }
      if (session.takenOver) return; // Idempotent — already taken over

      // Check terminal capacity BEFORE aborting
      if (tm.count() >= tm.MAX_TERMINALS) {
        sendTo(ws, { type: "error", text: "Terminal limit reached. Close a terminal first." });
        return;
      }

      // Abort running SDK query + clear pending state
      if (session.abortController) {
        try { session.abortController.abort(); } catch (e) {}
      }
      Object.keys(session.pendingPermissions || {}).forEach(function (k) {
        delete session.pendingPermissions[k];
      });
      Object.keys(session.pendingAskUser || {}).forEach(function (k) {
        delete session.pendingAskUser[k];
      });

      // Mark as taken over (atomic — single-threaded Node.js)
      session.takenOver = true;
      session.takeoverTermId = null;

      // Clamp cols/rows to safe range (Codex finding: untrusted client values)
      var tCols = Math.max(10, Math.min(500, parseInt(msg.cols, 10) || 120));
      var tRows = Math.max(2, Math.min(200, parseInt(msg.rows, 10) || 30));

      // Spawn PTY with argv (NO shell interpolation)
      // onExit pre-registered to avoid fast-exit race (Codex finding)
      var t = tm.createWithCommand(
        "claude", ["--resume", session.cliSessionId, "--cwd", cwd],
        {
          cols: tCols, rows: tRows, title: "Claude CLI",
          onExit: function () {
            session.takenOver = false;
            session.takeoverTermId = null;
            send({ type: "takeover_ended" });
            sm.broadcastSessionList();
          }
        }
      );
      if (!t) {
        session.takenOver = false;
        sendTo(ws, { type: "error", text: "Failed to create terminal for takeover" });
        return;
      }
      session.takeoverTermId = t.id;

      // Broadcast term_list first so frontend has the tab
      // Use send() not sendAndRecord() — takeover state is conveyed via session_list
      // flag, not history replay (Codex finding: avoids stale takenOver on reconnect)
      send({ type: "term_list", terminals: tm.list() });
      send({ type: "takeover_started", termId: t.id });
      sm.broadcastSessionList();
      return;
    }

    if (msg.type === "return_from_terminal") {
      var session = sm.getActiveSession();
      if (session && session.takenOver && session.takeoverTermId) {
        // close() fires exitCallbacks synchronously, which clears
        // takenOver and broadcasts takeover_ended. No fallback needed.
        tm.close(session.takeoverTermId);
      }
      return;
    }
    ...
    if (msg.type === "set_backend") {
      ...
      if (session.isProcessing) {
        sendTo(ws, { type: "error", text: "Cannot switch backend while processing. Stop the current query first." });
        return;
      }

      var newBackend = msg.backend;
      if (newBackend !== "claude" && newBackend !== "codex") return;

      if (newBackend === "codex") {
        var status = codex.isAvailable();
        if (!status.available) {
          sendTo(ws, { type: "error", text: "Codex unavailable: " + status.reason });
          return;
        }
      }

      session.activeBackend = newBackend;
      sm.saveSessionFile(session);
      send({ type: "backend_changed", backend: newBackend });
      sm.broadcastSessionList();
      return;
    }
    ...
    // Block messages while session is in terminal takeover
    if (session.takenOver) {
      sendTo(ws, { type: "error", text: "Session is active in terminal. Return to web chat first." });
      return;
    }

    log(slug, "user message (" + ((msg.text || "").length) + " chars" + (msg.images && msg.images.length ? ", " + msg.images.length + " images" : "") + ")");

    // Timestamp for turn timing instrumentation
    session.t_received = Date.now();
    ...
    if (!session.isProcessing) {
      ...
      if (session.activeBackend === "codex") {
        codex.startQuery(session, fullText, msg.images);
      ...
      if (session.activeBackend === "codex") {
        codex.pushMessage(session, fullText, msg.images);
      } else {
        sdk.pushMessage(session, fullText, msg.images);
      }
    }
  ...
  function handleHTTP(req, res, urlPath) {
    ...
    if (req.method === "POST" && urlPath === "/api/permission-response") {
      // CSRF protection: require JSON content type (browsers can't send
      // cross-origin JSON POST without CORS preflight, which we don't allow)
    ...
    if (req.method === "GET" && urlPath === "/api/codex/status") {
      var status = codex.getStatus();
      res.writeHead(200, { "Content-Type": "application/json" });
  ...
  return {
    ...
    destroy: destroy,
    getSessionData: function (localId) { return sm.getSessionData(localId); },
    removeSession: function (localId) { return sm.removeSession(localId); },
    exportSession: function (localId) { return sm.exportSession(localId); },
    importSession: function (data) { return sm.importSession(data); },
  };
}
...
function checkEndpoint(url, cb) {
  ...
  try {
    ...
    var isHttps = parsed.protocol === "https:";
    var req = (isHttps ? https : http).get({
      hostname: parsed.hostname,
      port: parsed.port || (isHttps ? 443 : 80),
      path: "/v1/models",
      timeout: 3000,
    ...
    (function (idx) {
      extractFirstUserMessage(entries[idx].filePath).then(function (title) {
        ...
        if (--pending === 0) {
          ...
          var keys = Object.keys(cliSessionCache);
          if (keys.length > CLI_CACHE_MAX) {
            keys.sort(function (a, b) { return cliSessionCache[a].ts - cliSessionCache[b].ts; });
            for (var k = 0; k < keys.length - CLI_CACHE_MAX; k++) {
              delete cliSessionCache[keys[k]];
            }
          }
          res.writeHead(200, { "Content-Type": "application/json" });
