...
function createSDKBridge(opts) {
    ...
    if (parsed.type === "stream_event" && parsed.event) {
      ...
      if (evt.type === "content_block_start") {
        ...
        if (block.type === "tool_use") {
          ...
          if (session.turnTiming) {
            session.turnTiming.toolActive[block.id] = { name: block.name, t_start: Date.now() };
          }
      ...
      if (evt.type === "content_block_delta" && evt.delta) {
        ...
        if (evt.delta.type === "text_delta" && typeof evt.delta.text === "string") {
          if (session.turnTiming && !session.turnTiming.t_first_token) {
            session.turnTiming.t_first_token = Date.now();
          }
      ...
      if (Array.isArray(content)) {
        for (var i = 0; i < content.length; i++) {
          ...
          if (block.type === "tool_result" && !session.sentToolResults[block.tool_use_id]) {
            if (session.turnTiming && session.turnTiming.toolActive[block.tool_use_id]) {
              var ta = session.turnTiming.toolActive[block.tool_use_id];
              if (session.turnTiming.tools.length < 200) {
                session.turnTiming.tools.push({
                  name: ta.name,
                  duration: Date.now() - ta.t_start,
                });
              }
              delete session.turnTiming.toolActive[block.tool_use_id];
            }
            var resultText = "";
      ...
      if (session.turnTiming) {
        var now = Date.now();
        var tt = session.turnTiming;
        timing = {
          total: now - tt.t_received,
          overhead: tt.t_query_start - tt.t_received,
          ttft: tt.t_first_token ? tt.t_first_token - tt.t_query_start : null,
          tools: tt.tools,
          toolCount: tt.tools.length,
          toolTime: tt.tools.reduce(function(s, t) { return s + t.duration; }, 0),
        };
        log("sdk", "timing: total=" + timing.total + "ms overhead=" + timing.overhead + "ms ttft=" + (timing.ttft || "n/a") + "ms tools=" + timing.toolCount + " (" + timing.toolTime + "ms)");
      }
      ...
      session.turnTiming = null;
      delete session.t_received;
  ...
  function handleCanUseTool(session, toolName, input, opts) {
    ...
    // ExitPlanMode gets 30 min (user needs to read the plan), others get 5 min
    var permTimeout = toolName === "ExitPlanMode" ? 1800000 : 300000;
  ...
  async function processQueryStream(session) {
    try {
      ...
      // Clean up any orphaned permission/askUser requests so they don't
      // re-appear as permission_request_pending on reconnect
      var pendingKeys = Object.keys(session.pendingPermissions || {});
      for (var pi = 0; pi < pendingKeys.length; pi++) {
        var pp = session.pendingPermissions[pendingKeys[pi]];
        sendAndRecord(session, { type: "permission_cancel", requestId: pendingKeys[pi] });
        if (pp && pp.resolve) pp.resolve({ behavior: "deny", message: "Query ended" });
      }
      session.pendingPermissions = {};
      session.pendingAskUser = {};
  ...
  async function startQuery(session, text, images) {
    ...
    session.turnTiming = {
      t_received: session.t_received || Date.now(),
      t_query_start: Date.now(),
      t_first_token: null,
      tools: [],
      toolActive: {},
    };
