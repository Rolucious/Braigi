...
var net = require("net");
var { loadConfig, saveConfig, configPath, socketPath, logPath, ensureConfigDir, isDaemonAlive, isDaemonAliveAsync, generateSlug, clearStaleConfig, loadRc, saveRc } = require("../lib/config");
...
for (var i = 0; i < args.length; i++) {
  if (args[i] === "-p" || args[i] === "--port") {
    ...
    console.log("Usage: braigi [-p|--port <port>] [--no-update] [--debug] [-y|--yes] [--shutdown]");
    console.log("       braigi --add <path>     Add a project to the running daemon");
    console.log("       braigi --remove <path>  Remove a project from the running daemon");
    console.log("       braigi --list            List registered projects");
    console.log("");
    console.log("Options:");
    console.log("  -p, --port <port>  Port to listen on (default: 27244)");
...
if (addPath !== null) {
  ...
  isDaemonAliveAsync(addConfig).then(function (alive) {
    if (!alive) {
      console.error("No running daemon. Start with: npx braigi");
      process.exit(1);
    }
...
function onDaemonDied() {
  ...
  log(a.dim + "     Run " + a.reset + "npx braigi" + a.dim + " to start again." + a.reset);
  log("");
  process.exit(0);
}
...
// ==============================
// Restore projects from ~/.braigi-rc
// ==============================
function promptRestoreProjects(projects, callback) {
  ...
  promptMultiSelect("Restore projects", items, function (selected) {
    // Remove unselected projects from ~/.braigi-rc
...
function setup(callback) {
  ...
  printLogo();
  log("");
  log(sym.pointer + "  " + a.bold + "Braigi" + a.reset + a.dim + "  ·  Unofficial, open-source project" + a.reset);
  log(sym.bar);
...
// ==============================
async function forkDaemon(extraProjects) {
  var ip = getLocalIP();
  ...
  if (!portFree) {
    ...
    log(a.dim + "Is another Braigi daemon running?" + a.reset);
    process.exit(1);
    return;
  }
  ...
  // Add restored projects (from ~/.braigi-rc)
...
function showMainMenu(config, ip) {
  ...
  sendIPCCommand(socketPath(), { cmd: "get_status" }).then(function (status) {
    ...
    function afterQr() {
      // Status line
      log("  " + a.dim + "braigi" + a.reset + " " + a.dim + "v" + currentVersion + a.reset + a.dim + " — " + url + a.reset);
      var parts = [];
      ...
      promptSelect("What would you like to do?", items, function (choice) {
        switch (choice) {
          ...
          case "exit":
            ...
            log("  " + a.dim + "Run " + a.reset + "npx braigi" + a.dim + " to come back here." + a.reset);
            log("");
            process.exit(0);
            break;
        }
        ...
        hint: [
          "Run npx braigi in other directories to add more projects.",
          "★ github.com/Rolucious/Braigi — Press s to star the repo",
        ],
        keys: [
          ...
          { key: "s", onKey: function () {
            try {
              var openCmd = process.platform === "darwin" ? "open" : "xdg-open";
              spawn(openCmd, ["https://github.com/Rolucious/Braigi"], { stdio: "ignore", detached: true }).unref();
            } catch (e) {}
            showMainMenu(config, ip);
          }},
        ],
...
function showSettingsMenu(config, ip) {
  ...
  log(sym.bar);
  log(sym.bar + "  Port         " + a.bold + config.port + a.reset);
  log(sym.bar + "  Debug        " + (config.debug ? a.green + "On" + a.reset : a.dim + "Off" + a.reset));
  log(sym.bar);
...
(async function () {
  ...
  if (alive) {
      ...
      promptSelect("Add " + a.green + slug + a.reset + " to relay?", [
        ...
        if (answer === "yes") {
          sendIPCCommand(socketPath(), { cmd: "add_project", path: cwd }).then(function (res) {
            ...
            log("");
            showMainMenu(config || { pid: status.pid, port: status.port }, ip);
          });
        } else {
          showMainMenu(config || { pid: status.pid, port: status.port }, ip);
        }
    ...
    if (autoYes) {
      ...
      await forkDaemon(autoRestorable.length > 0 ? autoRestorable : undefined);
    } else {
      setup(function () {
        // Check ~/.braigi-rc for previous projects to restore
        var rc = loadRc();
