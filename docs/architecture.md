...
```mermaid
graph TB
    CLI1["npx braigi<br/>(Terminal 1)"]
    CLI2["npx braigi<br/>(Terminal 2)"]
    IPC["Unix Socket<br/>daemon.sock"]
    Daemon["Daemon Process<br/>lib/daemon.js"]
    HTTP["HTTP/WS Server<br/>:27244"]
    P1["Project A"]
    P2["Project B"]
...
| `0.0.0.0` binding | Allows LAN access. Use a private network (Tailscale, VPN) for remote access |

## Session Storage

Sessions are stored at `.braigi/sessions/{cliSessionId}.jsonl` inside the project directory.
...
/p/{slug}/api/stt           → STT proxy (POST audio → Parakeet)
/p/{slug}/api/tts           → TTS proxy (POST text → Speaches)
/p/{slug}/api/voice/status  → Voice backend health check
/p/{slug}/api/cli-sessions  → Browse old Claude CLI sessions
/p/{slug}/api/...           → Project API (push, permissions, files)
```
...
Supported commands: `add_project`, `remove_project`, `get_status`, `set_project_title`, `shutdown`
